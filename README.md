...
внутрь, на целевой машине ставить ничего не нужно. Если на машине сборки нет
.NET SDK 9, скрипт скачает и поставит его **локально** в `./.dotnet`, не трогая
систему. Результат — самодостаточный каталог (по умолчанию
`~/Applications/KanbanFlow`), который можно передать другим и запускать.
...
Архитектура определяется автоматически (`osx-arm64`/`osx-x64`,
`linux-arm64`/`linux-arm`/`linux-x64`). Кросс-компиляции между платформами нет —
запускайте скрипт на целевом железе (и на том же типе CPU, что у получателей:
Apple Silicon ≠ Intel).

#### Запуск переданного каталога

Каталог самодостаточен, .NET ставить не нужно. Нужен тот же ОС/архитектура, под
которую он собран.

**macOS / Raspberry Pi:**

```bash
cd путь/к/KanbanFlow
./run.sh
```

Откроется браузер на `http://localhost:5200` (Raspberry Pi слушает ещё и сеть —
`http://<ip-пи>:5200` и `http://<hostname>.local:5200`). Останов — `Ctrl+C`.

На macOS `run.sh` сам снимает «карантин», если каталог прилетел архивом/AirDrop.
Если Gatekeeper всё же ругается на `KanbanFlowApi` — выполнить один раз:

```bash
xattr -dr com.apple.quarantine путь/к/KanbanFlow
```

На Raspberry Pi с рабочим столом есть ещё ярлык `KanbanFlow.desktop` (двойной
клик / пункт «KanbanFlow» в меню). С флагом `--service` при сборке приложение
поднимается при загрузке: `systemctl --user {status,restart} kanbanflow`,
логи — `journalctl --user -u kanbanflow -f`.
